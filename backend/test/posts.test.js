...
//             expect(res.body).to.have.property('title', 'New Test Post');
...
//         it('should update a particular field from a post, in this case title', async () => {
//             const res = await request(app)
//                 .put(`/posts/${initialPostId}`)
//                 .set('Authorization', `Bearer ${token}`)
//                 .send({
...
//             expect(res.status).to.equal(200);
//             expect(res.body).to.have.property('message', 'Post and associated comments removed');
//         });
